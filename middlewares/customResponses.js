const customResponses = {
	...
	mongooseError(error, next) {
		console.error(error);
		if (error.name === 'MongoError' && error.code === 11000) {
			return this.status(420).json({success: false,error: "There was a duplicate key error",});
		}
		else {
			return this.status(420).json({success: false, error: error});
			//return this.status(422).json({success: false, error: "something went wrong, try again later",});
		}
	}
};
